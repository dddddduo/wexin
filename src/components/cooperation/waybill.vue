<template>
  <div id="order">
    <div class="order-list">
      ...
      <div class="center" v-for="(item, index) in waybillList" :key="index">
        <ul>
          <li><span>运单名称：</span><p>{{ item.name ? item.name : '暂无数据' }}</p></li>
          <li><span>出单时间：</span><p>{{ item.outDate ? item.outDate : '暂无数据' }}</p></li>
        </ul>
        <div class="ul-title">物流信息</div>
        <ul>
          <li><span>承运企业：</span><p>{{ item.serviceCompany ? item.serviceCompany : '暂无数据' }}</p></li>
          <li><span>司机姓名：</span><p>{{ item.driver ? item.driver : '暂无数据' }}</p></li>
          <li><span>联系方式：</span><p>{{ item.phone ? item.phone : '暂无数据' }}</p></li>
          <li><span>身份证号：</span><p>{{ item.driverId ? item.driverId : '暂无数据' }}</p></li>
          <li><span>出发地：</span><p>{{ item.source ? item.source : '暂无数据' }}</p></li>
          <li><span>目的地：</span><p>{{ item.destination ? item.destination : '暂无数据' }}</p></li>
        </ul>
...
export default {
  ...
  beforeCreate () {
    document.getElementsByTagName('title')[0].innerText = '电子运单'
  },
  destroyed () {
  ...
  mounted () {
    ...
    that.$ajax.get('/sw/chain/coconfirm/transport/detail/?token=' + that.token + '&transportId=' + that.id + '&confirmId=' + that.confirmId).then(res => {
      if (res.data.code === 200) {
        that.waybillList = res.data.data.items
      }
    })
